% !TeX program = xelatex
% 2026 全国大学生数学建模竞赛论文骨架（电子版）
% 提交前请删除所有“待填写”提示，并核对 paper/README.md 中的清单。

\documentclass[withoutpreface,bwprint]{cumcmthesis}
\usepackage{cumcm2026}

\title{无线电干扰源自动定位与清除的几何建模及滚动决策}

\begin{document}

\maketitle

\input{sections/01_abstract}

% 官方规范明确要求正文不设目录，正文从电子版第 2 页开始。

\input{sections/02_problem_statement}
\input{sections/03_problem_analysis}
\input{sections/04_model_assumptions}
\input{sections/05_notation}

\section{模型建立与求解}

\input{sections/06_q1_model}
\input{sections/07_q2_model}
\input{sections/08_q3_model}
\input{sections/09_q4_model}

\input{sections/10_validation_results}
\input{sections/11_evaluation_improvement}

\input{sections/13_ai_statement}

\begin{thebibliography}{9}
  \bibitem{toussaint1983}
  Toussaint G T.
  \newblock Solving geometric problems with the rotating calipers[C/OL]//Proceedings of IEEE MELECON'83.
  \newblock Athens, Greece: IEEE, 1983: A10.02/1--4[2026-09-11].
  \newblock \url{https://www-cgrl.cs.mcgill.ca/~godfried/research/calipers.html}.

  \bibitem{fisher1922}
  Fisher R A.
  \newblock On the mathematical foundations of theoretical statistics[J].
  \newblock Philosophical Transactions of the Royal Society of London. Series A,
  1922, 222: 309--368. DOI: \url{https://doi.org/10.1098/rsta.1922.0009}.

  \bibitem{heldkarp1962}
  Held M, Karp R M.
  \newblock A dynamic programming approach to sequencing problems[J].
  \newblock Journal of the Society for Industrial and Applied Mathematics,
  1962, 10(1): 196--210. DOI: \url{https://doi.org/10.1137/0110015}.

  \bibitem{rockafellar2000}
  Rockafellar R T, Uryasev S.
  \newblock Optimization of conditional value-at-risk[J].
  \newblock The Journal of Risk, 2000, 2(3): 21--41.
  DOI: \url{https://doi.org/10.21314/JOR.2000.038}.

  \bibitem{cumcmthesis}
  LaTeX Studio.
  \newblock CUMCMThesis：全国大学生数学建模竞赛 \LaTeX{} 论文模板[CP/OL].
  \newblock 版本 2.9, 2026-08-26 [2026-09-10]. \url{https://github.com/latexstudio/CUMCMThesis}.

  % 按正文首次引用顺序继续添加文献；所有公开资料和网上资料均须在正文标注。
\end{thebibliography}

\clearpage
\begin{appendices}

\input{sections/14_supporting_materials}
\input{sections/15_source_code}

\end{appendices}

\end{document}
