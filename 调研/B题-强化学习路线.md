...
| 项目 | 建模口径 | 误用会导致什么 |
|---|---|---|
| 源 | 半径1800 m圆域内10—16个固定源；1—20频道、每频道至多一源 | 给策略真实总数会形成信息泄漏 |
| 信号 | 每源固定有效半径1000—1500 m；Q4可能有未知朝向的半平面覆盖 | 一次无信号不能判定频道为空 |
| 误差 | 示向度误差有界±1°，同位置对同源误差固定 | 原地重复测量不能按独立噪声平均消除误差 |
| 行走 | 5 m/s，直线距离计时，移动途中不自动检测；可超出目标圆域 | 不能把连续经过区域都记为已扫描 |
...
以下只分析一个**人为均匀分布玩具模型**，不代表官方地图分布。源在圆域均匀、随机选择频道、清除点也无信息时，命中特定频道且距离20 m以内的概率上界约为
...
|---|---|---|
| 几何 | 圆域1800 m、源静止、机器人可出域 | 均匀、成簇、近边界、对称布置分别测试；不是已知官方分布 |
