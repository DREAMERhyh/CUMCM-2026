...
- 不修改题目 PDF、附件或其他原始材料。
- 用户已授权新增 Q3 自适应策略、同点多频道联合批测，以及多源路线第一、二部分与离线配对测试；Q3 第二部分源码虽存在但正式默认未启用。Q4 已完成正确性基线、25 点扫描、有限联合后验、Q2 离散/连续 FIM/Pareto 候选接入、清除失败原地门控补测、方向探测组滚动、长清除尾救援，并可选接入 Q3 第一部分 insertion+2opt；Q3 beam/cache 未接入 Q4。
...
- Q2 保留原离散搜索在 `baseline.selected_point/selected`，连续结果放在 `continuous_fim`；两种选点都用原集合评分器复算。顶层 `selected_point/selected` 是按 `(R_wc,T)` 从两者作出的最终推荐。Q3 默认滚动模式会把顶层、离散、连续多预算、Pareto 与保证接收域候选合并重评。Q4 默认把这些点作为认证方向探测组的补充候选，再由自身联合方向场景与 Q4 Pareto 重评；它们不替代认证点，四侧探测不可用或求解失败时仍进入认证局部三角网或 27 m 后验清除网格。
...
- Q4 默认发现网为边长 985 m、晶格分数相位 `(0.112,0.112)` 的 25 点平移三角网；任一目标圆内源位于某个保留三角形内，三顶点距离均不超过 985 m，且任意发射半平面至少包含一顶点。机器人从原点出发的固定开放路线经 2-opt 后约 24552.150 m，覆盖余量 15 m。`triangular37` 与 `grid121` 保留为回归模式；25 点已完成几何证明、本地稠密检查和本轮 8 场候选接入配对，尚未完成官方模拟器验收。
- Q4 默认 `q2_candidate_mode="hybrid_pareto"`：Q2 离散、连续 FIM 多预算和 Pareto 点先按动作时间、`no_signal` 权重、Q2 后验半径取静态前沿，再与全部认证点一起由 Q4 联合场景重算方向分支、安全清除和续程成本并取 Q4 Pareto。候选缓存按位置后验和配置版本化，容量 256；FIM 子截止 0.75 s，候选生成与首次方向评价共享 3 s 软预算。`off` 保留回归；失败/超时不删认证点。
...
- 2026-09-13 Q2/FIM/Pareto 候选接入固定种子 20271913、8 个 25 点扫描与 insertion+2opt 场景中，开关两侧均 8/8 完整清除且无源证书正确；开启后平均虚拟时间 11562.962→11052.945 s（-4.410%），P90 13106.818→12349.732 s，6/8 更快，移动 48486.687→45482.226 m。失败清除 14.125→42.125，墙钟 25.064→75.518 s。队员明确只以虚拟时间下降裁决，因此默认开启 `hybrid_pareto`，其他退化保留披露。结果见 `output/q4_offline/q2_candidate_benchmark.{json,md}`。
- 2026-09-13 候选接入后 Q4/CLI 定向测试 21/21 通过，全项目 `python -B -m unittest discover -s tests -t . -v` 为 139/139 通过，耗时 94.395 s。过程中默认行为断言曾按旧“仅认证点”预期失败，修正为同时核对 Q2 候选实际入选和原认证点全集仍保留后通过；未发现实现错误。
...
- Q3/Q4：策略由 `runtime.runner.run_policy(policy, client)` 驱动；Q4 继承 Q3 的动作与清除状态机。`q4.candidate_planner` 提取 Q2 离散/FIM/Pareto 点并构造静态前沿，`q4.rolling.evaluate_directional_probe_decision` 再以联合方向场景构造 Q4 Pareto、对方向探测序列和长尾一步救援估值；`q4.directional.certified_probe_points` 始终提供连续证书。可选路线复用 `q3.route` 的服务块、最便宜插入和 2-opt，并以 `route_active_channel` 保证选中源由原 Q4 单源策略处理完成。
...
python -B tests/q4/benchmark_q2_candidates.py --cases 8 --seed 20271913 --workers 4 --max-actions 10000 --rolling-cpu-time-limit-s 3 --integrated-planning-cpu-time-limit-s 3 --q2-candidate-fim-cpu-time-limit-s 0.75 --route-cpu-time-limit-s 3 --output output/q4_offline/q2_candidate_benchmark.json
```
...
- **Implemented Q4 phases 1-8:** 完整探测组计数、动态四侧偏移、安全后验清除、25 点发现网、有限联合后验、原地补测、方向滚动、长尾救援、Q2/FIM/Pareto 补充候选和可选 insertion+2opt 多源路线均已完成。补充候选不替换认证点，`no_signal` 与超时仍回退认证组/安全清除。队员指定只按平均虚拟时间裁决，8 场景下降 4.410%，故候选默认开启；路线仍只排源间顺序并保持可选，Q3 beam/cache 未接入。
...
从 `快速上手指南.md` 查看结构和状态，从分题 README 找算法到代码的映射，从 API.md 获取测试数据契约。Q4 默认采用 25 点认证发现网和 `hybrid_pareto` Q2/FIM/Pareto 补充候选，长尾救援默认开启；候选接入在 8 场景中平均虚拟时间下降 4.410%，但失败清除和 wall-time 增加。Q4 insertion+2opt 仍是显式可选路线层，Q3 beam/cache 未接入。最新全项目回归 139/139 通过；官方模拟器验收未执行，Q1-Q4 仍为【待验证】。
