...
- 不修改题目 PDF、附件或其他原始材料。
- 用户已授权新增 Q3 自适应策略、同点多频道联合批测，以及多源路线第一、二部分与离线配对测试；Q3 第二部分源码虽存在但正式默认未启用。Q4 已完成正确性基线、37 点扫描、有限联合后验、清除失败原地门控补测、方向探测组滚动、长清除尾救援，并可选接入 Q3 第一部分 insertion+2opt；Q3 beam/cache 未接入 Q4。
...
- Q4 默认发现网已由 121 点改为边长 900 m 的 37 点三角网；任一目标圆内源位于某个保留三角形内，三顶点距离均不超过 900 m，且任意发射半平面至少包含一顶点。固定扫描开放路线经 2-opt 后为 36 段 900 m、总长 32400 m。`grid121` 保留为回归模式。
...
- 2026-09-13 全项目 `python -B -m unittest discover -s tests -t . -v` 为 137/137 通过，耗时 90.111 s；Q1-Q4 状态仍为【待验证】。
...
- **Implemented Q4 phases 1-7:** 完整探测组计数、动态四侧偏移、安全后验清除、37 点发现网、有限联合后验、原地补测、方向滚动、长尾救援和可选 insertion+2opt 多源路线均已完成。长尾方案只在同源连续 8 次清除失败后最多启动一次有限后验认可的移动探测；全局单步化、逐次失败重排、空间冷却和强制绕行因退化或无收益依据被否决。路线只排源间顺序并锁定单源服务，不接入 Q3 beam/cache。8 场景结果支持长尾默认开启、路线仅可选保留。
...
从 `快速上手指南.md` 查看结构和状态，从分题 README 找算法到代码的映射，从 API.md 获取测试数据契约。Q4 长尾救援默认开启；Q4 insertion+2opt 已实现但因 8 场景仅 6/8 胜且失败清除增加，默认保持关闭，可在离线或人工明确决定的在线运行中显式开启。Q3 beam/cache 没有接入 Q4。最新全项目回归 137/137 通过；当前环境没有官方模拟器，Q1-Q4 仍为【待验证】，不要把自动测试或离线配对理解为人工验收或官方模拟器成绩。
