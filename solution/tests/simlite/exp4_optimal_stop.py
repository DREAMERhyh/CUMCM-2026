"""[夜间自主] 阶段4：A3 最优停止 门1 同池对比。

臂 A= Q3Policy() 默认；臂 B= Q3Policy(use_optimal_stop=True)。
用法：python tests/simlite/exp4_optimal_stop.py [局数] [seed起始]
"""
...
def make_stop():
    return Q3Policy(use_optimal_stop=True)


def main():
    count = int(sys.argv[1]) if len(sys.argv) > 1 else 120
    seed_start = int(sys.argv[2]) if len(sys.argv) > 2 else 20900000
    workers = max(1, (os.cpu_count() or 4) - 2)
    seeds = list(range(seed_start, seed_start + count))
    started = time.perf_counter()
    payload = paired_compare(make_base, make_stop, seeds, workers=workers)
    payload["meta"] = {
        "arm_a": "baseline", "arm_b": "use_optimal_stop=True",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp4_optimal_stop.json"
