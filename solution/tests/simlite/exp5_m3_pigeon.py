...
def make_pigeon():
    return Q3BatchPolicy(pigeonhole_early_stop=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_pigeon, seeds, workers=workers)
    payload["meta"] = {
        "arm_a": "batch_default", "arm_b": "pigeonhole_early_stop=True",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp5_m3_pigeon.json"
