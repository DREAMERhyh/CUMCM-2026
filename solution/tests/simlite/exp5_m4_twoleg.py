"""[实验轮5-主攻1c] D2 两腿横断协议 快筛（30 局）。

臂 A= Q3BatchPolicy() 磁盘默认；臂 B= Q3BatchPolicy(two_leg_refine=True)。
"""
...
def make_twoleg():
    return Q3BatchPolicy(two_leg_refine=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_twoleg, seeds, workers=workers)
    payload["meta"] = {
        "arm_a": "batch_default", "arm_b": "two_leg_refine=True",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp5_m4_twoleg.json"
