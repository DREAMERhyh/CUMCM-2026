"""[实验轮3-P2] tour_refine（顺路最小绕行腿）vs 现状批量 FIM 腿，门1 快筛。

用法：python tests/simlite/exp3_p2_tour.py [局数] [seed起始]
"""
...
def make_base():
    return Q3BatchPolicy(use_optimal_stop=False, tour_refine=False)


def make_tour():
    return Q3BatchPolicy(use_optimal_stop=False, tour_refine=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_tour, seeds, workers=workers)
    payload["meta"] = {
        "arm_a": "batch(fim legs)", "arm_b": "batch(tour_refine legs)",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp3_p2_tour.json"
