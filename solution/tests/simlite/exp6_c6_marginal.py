"""[实验轮6-猜6] tour-aware 联合排序 快筛（30 局）。

臂 A= Q3BatchPolicy() 磁盘默认（M9 近邻序）；臂 B= Q3BatchPolicy(tour_aware_marginal=True)。
"""
...
def make_marginal():
    return Q3BatchPolicy(tour_aware_marginal=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_marginal, seeds,
                             workers=workers)
    payload["meta"] = {
        "arm_a": "near_order", "arm_b": "tour_aware_marginal",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp6_c6_marginal.json"
