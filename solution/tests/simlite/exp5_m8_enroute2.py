"""[实验轮5-主攻1e] M8 enroute v2（绕行判定）快筛（30 局）。

臂 A= Q3BatchPolicy() 磁盘默认；臂 B= Q3BatchPolicy(enroute_refine=True)
（v2：detour<=250m 判定 + 走廊/线段/直扑三候选）。
"""
...
def make_enroute2():
    return Q3BatchPolicy(enroute_refine=True, enroute_detour_m=250.0)
...
def main():
    ...
    payload = paired_compare(make_base, make_enroute2, seeds,
                             workers=workers)
    payload["meta"] = {
        "arm_a": "batch_default", "arm_b": "enroute_v2(detour=250m)",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp5_m8_enroute2.json"
