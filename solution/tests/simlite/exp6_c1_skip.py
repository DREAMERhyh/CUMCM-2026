"""[实验轮6-猜1'] 角度判据交织跳测 快筛（30 局）。

臂 A= Q3BatchPolicy() 磁盘默认（interleaved 照测全部已发现频道）；
臂 B= Q3BatchPolicy(skip_low_value_refine=True)（张角<30° 或 r<=15m 跳过）。
"""
...
def make_skip():
    return Q3BatchPolicy(skip_low_value_refine=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_skip, seeds, workers=workers)
    payload["meta"] = {
        "arm_a": "default_interleaved", "arm_b": "skip_low_value_refine",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp6_c1_skip.json"
