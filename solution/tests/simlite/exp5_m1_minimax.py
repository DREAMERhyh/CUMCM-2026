"""[实验轮5-主攻1] minimax refine 腿 快筛（30 局→同池 300）。

臂 A= Q3BatchPolicy() 磁盘默认（FIM refine）；臂 B= Q3BatchPolicy(minimax_leg=True)。
用法：python tests/simlite/exp5_m1_minimax.py [局数] [seed起始]
"""
...
def make_minimax():
    return Q3BatchPolicy(minimax_leg=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_minimax, seeds, workers=workers)
    payload["meta"] = {
        "arm_a": "batch_default(fim_refine)", "arm_b": "minimax_leg=True",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp5_m1_minimax.json"
