"""[实验轮6-猜5] 巡游节点 guess-clear 快筛（30 局）。

臂 A= Q3BatchPolicy() 磁盘默认；臂 B= Q3BatchPolicy(tour_guess=True)。
"""
...
def make_tourguess():
    return Q3BatchPolicy(tour_guess=True)
...
def main():
    ...
    payload = paired_compare(make_base, make_tourguess, seeds,
                             workers=workers)
    payload["meta"] = {
        "arm_a": "default", "arm_b": "tour_guess=True",
        "count": count, "seed_start": seed_start, "workers": workers,
        "elapsed_s": round(time.perf_counter() - started, 1),
    }
    out = Path(__file__).with_name("analysis") / "exp6_c5_tourguess.json"
