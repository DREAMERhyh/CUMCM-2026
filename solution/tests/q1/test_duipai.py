...
Reads deterministic cases produced by gen_data.py, re-derives every bearing from
...
def _observations(case):
    data = case["input"]
    return [dict(position=point, status="direction", bearing_deg=bearing)
            for point, bearing in zip(data["detector_points"], data["bearings_deg"])]


def _check_data(case):
    checks = []
    sx, sy = case["oracle"]["source_position"]
    checks.append(("源在分布域内", math.hypot(sx, sy) <= DOMAIN_RADIUS + 1e-6,
                   f"r={math.hypot(sx, sy):.6f}"))
    for i, detector in enumerate(case["input"]["detector_points"]):
        x, y = detector
        distance = math.hypot(sx - x, sy - y)
        checks.append((f"检测点{i+1}不与源重合", distance > 5.0,
                       f"d={distance:.6f}"))
        true = math.degrees(math.atan2(sy - y, sx - x)) % 360.0
        measured = case["input"]["bearings_deg"][i]
        delta = _wrap(measured - true)
        checks.append((f"示向度{i+1}误差在±1°内", abs(delta) <= 1.0 + 1e-9,
                       f"error={delta:.9f}"))
        checks.append((f"示向度{i+1}与记录误差一致",
                       abs(delta - case["oracle"]["bearing_errors_deg"][i]) <= 2e-9,
                       f"{delta:.9f} vs {case['oracle']['bearing_errors_deg'][i]:.9f}"))
    return checks
...
def _check_region(case, region):
    ...
    if region["status"] != "bounded":
        if len(case["input"]["detector_points"]) == 1:
...
def check_case(case):
    ...
    try:
        region = analyze_q1(case["input"]["detector_points"],
                            case["input"]["bearings_deg"],
                            case["input"]["error_deg"])["region"]
    ...
    for case in cases:
        ...
        try:
            region_status = analyze_q1(
                case["input"]["detector_points"],
                case["input"]["bearings_deg"],
                case["input"]["error_deg"])["region"]["status"]
        except Exception:
            region_status = "error"
