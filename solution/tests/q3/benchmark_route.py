...
STRATEGIES = ("current", "insertion_2opt")
...
def _policy(strategy, fim_limit_s, rolling_limit_s, route_limit_s):
    return InstrumentedRoutePolicy(
        ...
        multi_source_route_mode=(
            "insertion_2opt" if strategy == "insertion_2opt" else "off"
        ),
...
def _run_one(task):
    case, strategy, fim_limit_s, rolling_limit_s, route_limit_s = task
    policy = _policy(strategy, fim_limit_s, rolling_limit_s, route_limit_s)
    sources = [
...
def _markdown(report):
    ...
    for label, key in (
        ...
        lines.append(f"| {label} | {current[key]} | {route[key]} |")
    lines.extend([
        "",
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Q3多源服务块、最便宜插入与2-opt同场景配对"
    )
    ...
    parser.add_argument(
        "--output", default="output/q3_offline/route_benchmark.json"
    )
    ...
    if (args.cases < 1 or args.workers < 1
        ...
        parser.error("案例数、进程数和三个CPU时限必须为正数。")
