...
扫描停点布局（``SCAN_LAYOUTS``）融合自实验分支：候选布局附带解析覆盖
验证（``hub_ring6_analytical_bounds``/``pure_ring8_analytical_bounds``）
与数值网格核验（``verify_coverage_numerically``），以及按官方时间模型
拆分的扫描段虚拟时间（``scan_phase_virtual_time``）。
"""
...
def maximum_coverage_distance(points, *, rho_step=5.0, theta_step_deg=0.5,
    ...
    for index_rho in range(rho_count):
        ...
        for index_theta in range(theta_count):
            ...
            distance = nearest_coverage_distance(point, centers=points)
            if distance > worst:
                worst = distance
                worst_point = point
    return {
