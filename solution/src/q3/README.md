...
第二部分先在训练种子 20264912 的 1 个场景比较束宽 1/2/4：三者虚拟时间相同，束宽 1 墙钟最低，因此锁定束宽 1。随后在第一部分相同种子 20263912 的 4 个场景做三策略配对，三者均 4/4 完成。`beam_cached` 相对 `insertion_2opt` 为 0/4 胜，平均总虚拟时间 5236.592→5417.769 s（增加 181.176 s），P90 由 5876.333 s 恶化到 5918.775 s，平均跨源移动增加 1306.861 m；缓存命中率 77.9%，每场平均扩展 841.25 个节点，平均真实墙钟由 206.465 s 降到 194.052 s。按照预先约定的判据，缓存确实减少了部分计算耗时，但束搜索没有稳定优于第一部分，因此当前正式候选仍是 `insertion_2opt`，`beam_cached` 仅保留为实验模式。结果见 `output/q3_offline/route_part2_beam_train.json` 与 `route_part2_benchmark.{json,md}`。
