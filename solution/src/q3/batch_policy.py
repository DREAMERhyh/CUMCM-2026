...
融合自实验分支（B2-exploration-hyh）：先完成全部源的定位（certificate
级），再用精确 TSP 规划清除顺序（n<=16），与现有"交错"流程同池对比。
本策略独立于交错的 Q3Policy 状态机（仅继承共性方法），不改变交错路径的
任何逻辑。实验分支轮 3/5 已证伪的腿选择器（minimax/tour/two_leg）不并入。

默认构造参数与实验分支线上冻结配置对齐：scan_layout="pure_ring8"、
max_refinements=2、fim_cpu_time_limit_s=6.0、use_optimal_stop=True。
"""
...
class Q3BatchPolicy(Q3Policy):
    ...
    def __init__(self, *, tspn_clear=True, enroute_refine=False,
                 enroute_detour_m=250.0, batch_guess=False, **kwargs):
        kwargs.setdefault("scan_layout", "pure_ring8")
        kwargs.setdefault("max_refinements", 2)
        kwargs.setdefault("fim_cpu_time_limit_s", 6.0)
        kwargs.setdefault("use_optimal_stop", True)
    ...
    def _locate_step(self, state, remaining):
        if remaining:
            ...
            if radius <= 19.9:
                ...
                return None
            # batch_guess（默认关闭）：批量 locate 的 guess 窗口——hit 省
            # 一条 refine 专腿，miss 只花 3s；实验分支快筛 25/30 更差
            # （"专程去中心再回来"双程成本高于直接 refine），保留开关。
            if (self.batch_guess
    ...
    def _enroute_refine_pending(self, state):
        ...
        源先顺路 1 腿。实验轮5 快筛 30/30 平局（难源与 TSP 路径空间不相
        近），保留开关供显式配置复用。"""
    ...
    def apply_response(self, state, action, response):
        ...
        if mode not in ("batch_refine", "batch_clear", "enroute_refine"):
