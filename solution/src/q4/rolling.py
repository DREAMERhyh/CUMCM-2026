...
by the certified probe sets and the finite clear cover.

本文件移植自 main 分支基线，并把 main 独有模块（q3.adaptive 的
posterior_clear_points/nearest_neighbor_order、q3.joint 的
predicted_clear_cost）内联为模块内实现，使 B2 分支的 q3 保持零改动。
"""
...
# ---- 内联自 main 分支 q3/adaptive.py（B2 分支 q3 无此模块）----
...
def posterior_clear_points(region, *, spacing_m=27.0):
    """27m 方格覆盖当前凸源区域（覆盖半径 19.092m，留 0.908m 余量）。"""
