...
`--fim-cpu-time-limit-s 10` 是 Q3 每次 Q2 连续 FIM 规划允许的真实 CPU 墙钟上限，不是机器狗虚拟动作时间。Q3 默认关闭近优域绘图计算，因此该时限只用于产生实际细化测点。Q4 的候选接入另用默认 0.75 s 连续 FIM 子截止；Q2 点只作认证探测组补充，不作为不安全的单点回退。
...
`--q4-q2-candidate-mode hybrid_pareto` 默认把 Q2 离散、连续 FIM 多预算和 Pareto 点加入认证探测组，再由 Q4 联合方向场景与 Q4 Pareto 重新裁决；`off` 保留为回归。默认候选 FIM 子截止为 0.75 s，候选生成与首次方向评价共享 3 s 软预算。8 场景中平均虚拟时间下降 4.410%，因此按队员指定的“只看虚拟时间”标准默认开启；真实 wall-time 增长仍会出现在诊断中。
