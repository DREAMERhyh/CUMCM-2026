...
Each circle starts from a coarse circumscribed regular polygon.  Feasible
line-circle and circle-circle intersections seed tangents at curved-boundary
endpoints, then violating polygon vertices add further tangents until the
requested side count's doubled-resolution radial tolerance is met.  Every
added half-plane contains the corresponding disk, so refinement can tighten
the polygon without excluding a physically possible source.
"""
...
def extend_region_with_observation(region, observation, *, error_deg=1.005,
                                   max_receive_radius=1500.0,
                                   circle_sides=32):
    """Intersect an existing polygon with one adaptively modelled observation."""
    if region.get("status") != "bounded":
    ...
    if region.get("approximation", {}).get("kind") != (
