...
模拟器通过 `POST /measure` 提供检测位置对应的 `measure_result`；仅当结果为 `direction` 时提供 `[0,360)` 内、保留两位小数的 `svd_deg`。运行层将其映射为本模块使用的检测位置与示向度；本模块内部生成半平面、顶点、直径及圆覆盖结论。官方请求、响应、幂等和串行时序由 `src/sim/` 统一实现，本模块不直接发送网络请求。
